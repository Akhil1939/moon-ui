...
export class GridColumnComponent {
  ...
  @Input() dataSource: CommonMatTableDataSource<any> =
    new CommonMatTableDataSource<any>();
  @ViewChildren(MatColumnDef) matColumnDef!: QueryList<MatColumnDef>;
  @ViewChild('columnReference', { static: false })
  columnReference!: ColumnReference;
  @ViewChild(MatHeaderCellDef, { static: false })
  ...
  updateColumnDefs() {
    this.matColumnDef.forEach((columnDef) => {
      if (columnDef.name !== 'select' && columnDef.name !== 'radio') {
        columnDef.headerCell =
          this.columnReference.sccHeaderCell.matHeaderCellDef;
      ...
      if (
        !(
          this.matTable &&
          this.matTable['_columnDefsByName'] &&
          this.matTable['_columnDefsByName'].get(columnDef.name)
        )
      ) {
