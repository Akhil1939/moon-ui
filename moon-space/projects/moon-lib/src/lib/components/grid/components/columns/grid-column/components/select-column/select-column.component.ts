...
export class SelectColumnComponent implements OnInit {
  ...
  toggleAllRows() {
    if (this.isAllSelected()) {
      ...
      this.selection.select(
        ...this.dataSource.data.map((row) => row[this.key]),
      );
    }
  }
    ...
    this.selection.changed
      ...
      .subscribe((selection) => {
        if (this.column.selectionChange) {
          ...
          const selectedRows = this.dataSource.data.filter((item) =>
            this.selection.isSelected(item[this.key]),
          );
  ...
  newDataHasSelected() {
    return (
      this.dataSource.data.some((e) =>
        this.selection.isSelected(e[this.key]),
      ) &&
      this.dataSource.data.filter((e) => this.selection.isSelected(e[this.key]))
        .length < this.dataSource.data.length
    );
